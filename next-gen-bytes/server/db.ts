...
function initLocalDb() {
  try {
    ...
    console.warn("⚠️ Warning: Read-only filesystem detected. Next Gen Bytes will fall back to secure in-memory lead storage.");
  }
}
...
function readLocalInquiries(): IInquiry[] {
  try {
    ...
    console.warn("⚠️ Failed to read local fallback DB file, using session memory:", err.message);
  }
  return memoryInquiries;
}

function writeLocalInquiries(inquiries: IInquiry[]) {
  // Always update our active memory cache
  ...
  try {
    ...
    console.warn("⚠️ Failed to persist fallback DB to file, preserved in memory:", err.message);
  }
}
